...
import { GlassCard, PageShell, SectionHeader } from "@/components/PageShell";
    ...
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Your peak cognitive window starts in <span className="text-coral">42 minutes</span>.
        </p>
      </div>
      <div className="flex items-center gap-2">
        <button className="px-4 py-2 rounded-xl glass text-sm hover:bg-white/5 transition">
          View report
        </button>
  ...
  return (
    <GlassCard>
      <SectionHeader
        ...
        sub="Live signals from the last 24 hours"
        action={
          <span className="text-xs px-2 py-1 rounded-full bg-coral/15 text-coral">Optimal</span>
        }
      />
...
function FocusTimer() {
  ...
  return (
    <GlassCard className="relative overflow-hidden">
      ...
      <SectionHeader title="Focus timer" sub="Deep work · session 2 of 4" />
      <div className="relative flex flex-col items-center py-4">
        <div className="relative h-48 w-48">
          <div
            className={`absolute inset-0 rounded-full border-2 border-dashed border-coral/30 ${running ? "animate-spin" : ""}`}
            style={{ animationDuration: "20s" }}
          />
          <div
            className={`absolute inset-3 rounded-full bg-gradient-to-br from-coral/20 to-electric/20 ${running ? "animate-breathe" : ""}`}
          />
          <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <div className="mt-6 flex items-center gap-3">
          <button
            ...
            className="h-12 w-12 rounded-full bg-foreground text-background flex items-center justify-center hover:scale-105 transition"
          >
            {running ? <Pause className="h-5 w-5" /> : <Play className="h-5 w-5 ml-0.5" />}
          </button>
          <button className="h-12 px-5 rounded-full glass text-sm flex items-center gap-2">
...
function MoodTracker() {
  ...
  return (
    <GlassCard>
      <SectionHeader title="How do you feel?" sub="The system adapts to your state" />
      ...
      <div className="mt-3 text-xs text-muted-foreground text-center capitalize">
        Currently feeling: <span className="text-foreground">{mood}</span>
      </div>
...
function AdaptivePlanner() {
  ...
  const toggleTask = useCallback((taskId: string) => {
    setTasks((previousTasks) =>
      previousTasks.map((task) => (task.id === taskId ? { ...task, done: !task.done } : task)),
    );
  }, []);
  ...
  return (
    <GlassCard>
      <SectionHeader
        ...
        sub="Reordered by energy curve · updated 2m ago"
        action={
          <button className="text-xs px-3 py-1.5 rounded-full bg-coral/15 text-coral flex items-center gap-1">
            <Sparkles className="h-3 w-3" /> Re-optimize
          </button>
        }
      />
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                <span className="flex items-center gap-1">
                  <Flame
                    className={`h-3 w-3 ${t.priority === "high" ? "text-coral" : t.priority === "med" ? "text-electric" : ""}`}
                  />{" "}
                  {t.priority}
                </span>
                <span className="flex items-center gap-1">
                  <Zap className="h-3 w-3" /> {t.energy} energy
                </span>
                <span>{t.duration}</span>
              </div>
            </div>
            <button className="opacity-0 group-hover:opacity-100 transition text-xs px-2 py-1 rounded-md bg-white/5">
              Defer
            </button>
          </motion.div>
        ))}
...
function ProductivityAnalytics() {
  const heatmap = useMemo(() => cognitiveHeatmap, []);
  return (
    <GlassCard>
      <SectionHeader
        title="Productivity analytics"
        sub="Patterns across the week"
        action={
          ...
          <div className="grid grid-cols-12 gap-1">
            {heatmap.map((c) => (
              <div
                ...
                style={{
                  background: `color-mix(in oklab, var(--coral) ${Math.round(c.v * 100)}%, var(--surface-2))`,
                }}
              />
            ))}
          </div>
            ...
            <div className="flex gap-1">
              {[10, 30, 60, 90].map((v) => (
                <div
                  ...
                  style={{
                    background: `color-mix(in oklab, var(--coral) ${v}%, var(--surface-2))`,
                  }}
                />
              ))}
            </div>
    ...
    <GlassCard className="relative overflow-hidden">
      ...
      <div className="mt-4 flex flex-wrap gap-1.5">
        {["What should I study now?", "Am I overloaded?", "Optimize schedule"].map((s) => (
          <button
            key={s}
            className="text-xs px-3 py-1 rounded-full bg-surface/60 border border-white/5 hover:bg-white/5 transition"
          >
            {s}
          </button>
        ))}
      ...
      <form
        onSubmit={async (e) => {
          ...
          setInput("");
          const completion = await aiClient.complete([{ role: "user", content: text }]);
          setMessages((current) => [...current, { from: "ai", text: completion.text }]);
        }}
        className="mt-3 flex items-center gap-2 p-2 rounded-2xl bg-surface/60 border border-white/10 focus-within:border-coral/40 transition"
      >
        <input
          value={input}
  ...
  return (
    <div className="relative rounded-3xl p-6 bg-gradient-to-br from-[oklch(0.12_0.01_270)] to-[oklch(0.18_0.02_270)] border border-white/5 shadow-elevated overflow-hidden">
      ...
      <div className="relative">
        <SectionHeader title="Deep work" sub="Distraction-free since 9:14 AM" />
...
function HabitPanel() {
  return (
    <GlassCard>
      <SectionHeader title="Habits" sub="Reinforcement score 88/100" />
        ...
        {habits.map((h) => (
          <div key={h.name}>
            ...
            <div className="mt-1.5 h-1.5 rounded-full bg-surface-2 overflow-hidden">
              <div
                className="h-full rounded-full bg-gradient-to-r from-coral to-electric transition-all"
...
function CognitiveLoadCard() {
  const wave = useMemo(() => cognitiveLoadWave, []);
  return (
    <GlassCard>
      <SectionHeader
        title="Cognitive load"
        sub="Burnout risk: low"
        action={
      ...
      <div className="grid grid-cols-3 gap-2 mt-2 text-xs text-muted-foreground">
        <div>
          Workload <div className="text-foreground text-sm mt-0.5">62%</div>
        </div>
        <div>
          Pressure <div className="text-foreground text-sm mt-0.5">Med</div>
        </div>
        <div>
          Complexity <div className="text-foreground text-sm mt-0.5">High</div>
        </div>
      </div>
    </GlassCard>
  );
...
function LearningPanel() {
  return (
    <GlassCard>
      <SectionHeader title="Learning" sub="Spaced repetition · retention 84%" />
      <div className="space-y-2">
        {[
