...
import {
  Brain, Sparkles, Activity, Heart, Target, BookOpen,
  Gauge, Waves, ArrowRight, Play, Check, Star
} from "lucide-react";
import { Logo } from "@/components/Logo";
import {
  ResponsiveContainer, AreaChart, Area, XAxis, YAxis,
  RadialBarChart, RadialBar, LineChart, Line, Tooltip
} from "recharts";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Adaptive AI productivity system inspired by cognitive science. Focus regulation, fatigue awareness, intelligent prioritization." },
      { property: "og:title", content: "NeuroFlow AI" },
      { property: "og:description", content: "An adaptive AI productivity system inspired by cognitive science and brain behavior." },
    ],
  }),
  component: Landing,
});
...
const features = [
  { icon: Brain, title: "Adaptive Cognitive Planner", desc: "Reorders your day around energy peaks and recovery windows." },
  { icon: Gauge, title: "Fatigue-Aware Focus Engine", desc: "Detects mental drift and adapts session intensity in real time." },
  { icon: Heart, title: "Emotion-Adaptive Scheduling", desc: "Mood-aware planning that respects how you actually feel." },
  { icon: Sparkles, title: "AI Decision Assistant", desc: "Conversational coach that prioritizes, defers and protects focus." },
  { icon: Activity, title: "Neural Productivity Analytics", desc: "Patterns across focus, recovery and cognitive throughput." },
  { icon: Target, title: "Habit Reinforcement", desc: "Adaptive streak system tuned to your behavioral signals." },
  { icon: Waves, title: "Cognitive Load Balancing", desc: "Brainwave-style visualizations prevent burnout before it starts." },
  { icon: BookOpen, title: "Deep Work Optimization", desc: "Spaced repetition and retention scoring for compounding learning." },
];
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <header className="fixed top-4 inset-x-4 z-50 flex justify-center">
        <nav className="glass-strong rounded-full px-3 py-2 flex items-center gap-2 max-w-3xl w-full shadow-soft">
          ...
          <div className="hidden md:flex items-center gap-1 ml-4 text-sm text-muted-foreground">
            <a href="#features" className="px-3 py-1.5 rounded-full hover:text-foreground hover:bg-white/5 transition">Features</a>
            <a href="#how" className="px-3 py-1.5 rounded-full hover:text-foreground hover:bg-white/5 transition">How it works</a>
      ...
      <section id="features" className="py-32 px-6 relative">
        <div className="max-w-6xl mx-auto">
          <SectionHeader eyebrow="Brain-inspired modules" title="Cognitive infrastructure for deep workers" />
                  ...
                  <div className="mt-4 h-12 opacity-70">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={focusData}>
                        <Line type="monotone" dataKey="focus" stroke="var(--coral)" strokeWidth={1.5} dot={false} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
      ...
      <section id="how" className="py-32 px-6 relative">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mt-16 grid md:grid-cols-3 gap-4 relative">
            {[
              { n: "01", t: "Analyze behavior", d: "Continuous signals: focus, fatigue, mood, time-of-day patterns." },
              { n: "02", t: "Adapt workflow", d: "Schedule, breaks and priorities re-balance every few minutes." },
              { n: "03", t: "Optimize productivity", d: "Reinforced habits, deeper focus, measurable cognitive gains." },
            ].map((s) => (
          ...
          <div className="mt-16 grid md:grid-cols-3 gap-4">
            <div className="md:col-span-2 p-6 rounded-2xl glass">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <div className="text-3xl font-display mt-1">36.3<span className="text-sm text-muted-foreground"> hrs</span></div>
                </div>
              ...
              <div className="h-56">
                <ResponsiveContainer>
                  <AreaChart data={focusData}>
                    ...
                    <Area type="monotone" dataKey="focus" stroke="var(--coral)" strokeWidth={2} fill="url(#g1)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            <div className="p-6 rounded-2xl glass flex flex-col">
              ...
              <div className="flex-1 -mx-2">
                <ResponsiveContainer>
                  <RadialBarChart innerRadius="50%" outerRadius="100%" data={[{ name: "score", value: 86, fill: "var(--coral)" }]} startAngle={90} endAngle={-270}>
                    <RadialBar background={{ fill: "var(--surface-2)" } as object} dataKey="value" cornerRadius={20} />
                  </RadialBarChart>
                </ResponsiveContainer>
              </div>
      ...
      <section className="py-32 px-6">
        <div className="max-w-6xl mx-auto">
          <SectionHeader eyebrow="Loved by deep workers" title="Built for builders, scientists, and creatives" />
          <div className="mt-16 grid md:grid-cols-3 gap-4">
            {[
              { n: "Lena Park", r: "ML Researcher, Stanford", q: "It's the first tool that respects how my brain actually works across a week." },
              { n: "Marc Whitfield", r: "Founder, Atlas Labs", q: "Like having a chief of staff for my prefrontal cortex." },
              { n: "Aïsha Diallo", r: "Author & Speaker", q: "Quietly intelligent. Removed every interruption I didn't know I had." },
            ].map((t) => (
              <div key={t.n} className="p-6 rounded-2xl glass">
                <div className="flex gap-0.5 text-coral">{Array.from({ length: 5 }).map((_, i) => <Star key={i} className="h-3.5 w-3.5 fill-current" />)}</div>
        ...
        <div className="max-w-5xl mx-auto relative overflow-hidden rounded-3xl p-16 md:p-24 text-center glass-strong">
          ...
          <div className="relative">
            ...
            <div className="mt-6 flex items-center justify-center gap-6 text-xs text-muted-foreground">
              <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-coral" /> No credit card</span>
              <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-coral" /> 14-day trial</span>
            </div>
          </div>
        </div>
      </section>
...
function NeuralBg() {
  return (
    <svg className="absolute inset-0 w-full h-full opacity-[0.15] pointer-events-none" viewBox="0 0 800 600">
      {Array.from({ length: 20 }).map((_, i) => {
        const x1 = (i * 47) % 800, y1 = (i * 83) % 600;
        const x2 = ((i + 3) * 71) % 800, y2 = ((i + 5) * 53) % 600;
        return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="white" strokeWidth="0.5" />;
      })}
      {Array.from({ length: 30 }).map((_, i) => (
        <circle key={i} cx={(i * 53) % 800} cy={(i * 67) % 600} r="2" fill="var(--coral)" opacity={0.6} />
      ))}
    </svg>
  );
}
      ...
      <div className="rounded-2xl bg-background/60 p-6">
        ...
        <div className="mt-3 grid md:grid-cols-3 gap-3">
            ...
            <div className="h-36 -mx-2 mt-1">
              <ResponsiveContainer>
                <AreaChart data={focusData}>
                  ...
                  <Area type="monotone" dataKey="focus" stroke="var(--electric)" strokeWidth={2} fill="url(#hg)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          <div className="p-5 rounded-2xl bg-gradient-to-br from-coral/20 to-violet/20 border border-white/10 h-48 flex flex-col justify-between">
            ...
            <div className="text-sm leading-relaxed">Defer 2 low-priority tasks. Your fatigue trend is rising — protect 3:00 PM for shallow work only.</div>
            <button className="self-start text-xs px-3 py-1.5 rounded-full bg-foreground text-background">Apply</button>
          </div>
        </div>
      </div>
    </div>
...
function MiniStat({ label, value, accent }: { label: string; value: string; accent: "coral" | "electric" | "violet" }) {
